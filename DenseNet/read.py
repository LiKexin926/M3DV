...
def new_train(filepath):
    ...
    new_train = np.zeros(((1,32,32)))
    for meb in train_cropedtrans:
        new_train = np.vstack((new_train,meb))
    new_train = new_train[1:]
    new_train = new_train.reshape(2*len(train_del), 32,32,32,1) 
...
def new_test(csv_filepath,filepath):
    ...
    new_test = np.zeros(((1,32,32)))
    for meb in test_croped:
        new_test = np.vstack((new_test,meb))
    new_test = new_test[1:]
    
    new_test = new_test.reshape(len(test_del), 32,32,32, 1) 
